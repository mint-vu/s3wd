...
from utils import TwoAugUnsupervisedDataset, AverageMeter, strfdelta
...
class Options:
    ...
    # Meta settings
    method: str = "ssw" #Field(choices=["ssw", "simclr", "hypersphere"],
...
def pretrain(opt: Options):
    ...
    def sw_loss(x, y):
        ...
        unif_loss_val = (
            swd(x, x0, p=2, n_projs=opt.num_projections, device=x.device) ** 0.5 + 
            swd(y, y0, p=2, n_projs=opt.num_projections, device=y.device) ** 0.5
        ) / 2
    ...
    def s3w_loss(x, y):
        ...
        unif_loss_val = (
            s3wd(x, x0, p=2, n_projs=opt.num_projections, device=x.device) ** 0.5 + 
            s3wd(y, y0, p=2, n_projs=opt.num_projections, device=y.device) ** 0.5
        ) / 2
        return align_loss_val, unif_loss_val
    ...
    def ri_s3w_loss(x, y):
        ...
        unif_loss_val = (
            ri_s3wd(x, x0, p=2, n_projs=opt.num_projections, n_rotations=opt.num_rotations, device=x.device) ** 0.5 + 
            ri_s3wd(y, y0, p=2, n_projs=opt.num_projections, n_rotations=opt.num_rotations, device=y.device) ** 0.5
        ) / 2
        return align_loss_val, unif_loss_val
    ...
    def ari_s3w_loss(x, y):
        ...
        unif_loss_val = (
            ari_s3wd(x, x0, p=2, n_projs=opt.num_projections, n_rotations=opt.num_rotations, pool_size=opt.pool_size, device=x.device) ** 0.5 + 
            ari_s3wd(y, y0, p=2, n_projs=opt.num_projections, n_rotations=opt.num_rotations, pool_size=opt.pool_size, device=y.device) ** 0.5
        ) / 2
        return align_loss_val, unif_loss_val
