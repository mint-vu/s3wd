...
def ri_s3wd(X, Y, p, h=None, n_projs=1000, n_rotations=1, R=None, device='cpu'):
    """armotized option"""
    if h is None: h = hStar()
    
    if R is not None:
        assert n_rotations <= R.size(0)
        indices = torch.randperm(R.size(0))[:n_rotations]
        rot_matrices = R[indices].to(device)
    else:
        n = X.shape[-1]
        rot_matrices = [geotorch.SO(torch.Size([n, n])).sample('uniform') for _ in range(n_rotations)]
        rot_matrices = torch.stack(rot_matrices).to(device)
