...
def ri_s3wd(X, Y, p, h=None, n_projs=1000, n_rotations=1, presampled_rot_matrices=None, device='cpu'):
    # NOTE: h must accept vectors of the form (n_rotations, n_points, dim)
    if h is None: h = hStar()
    
    if presampled_rot_matrices is not None:
        n_samples = presampled_rot_matrices.shape[0]
        assert n_rotations <= n_samples
        indices = torch.randperm(n_samples)[:n_rotations]
        rot_matrices = presampled_rot_matrices[indices].to(device)
    else:
        n = X.shape[-1]
